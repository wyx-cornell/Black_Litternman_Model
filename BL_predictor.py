...
def example():
     ...
     df_gru = factor_return_rnn_predictor(deepcopy(df.shift(1).dropna(axis=0)),
                                        ...
                                        look_back=10, rnn=None,
                                        type='gru', num_internal_projection=10,dropout_probability = 0.2,
                                        init ='he_uniform', loss='mse', optimizer='rmsprop',
                                        nb_epoch=20, batch_size = 10, save_to_csv=None,
                                        train_freq="Monthly", train_period=365, verbosity=True)
     df_gru.to_csv('prediction_gru_v2.csv')

     df_lstm = factor_return_rnn_predictor(deepcopy(df.shift(1).dropna(axis=0)),
                                                 start_date=datetime(2006,5,1),
                                                 look_back=10, rnn=None,
                                                 type='lstm', num_internal_projection=10,dropout_probability = 0.2,
                                                 init ='he_uniform', loss='mse', optimizer='rmsprop',
                                                 nb_epoch=20, batch_size = 10, save_to_csv=None,
                                                 train_freq="Monthly", train_period=365, verbosity=True)
     df_lstm.to_csv('prediction_lstm_v2.csv')

     df_simple_RNN = factor_return_rnn_predictor(deepcopy(df.shift(1).dropna(axis=0)),
                                      start_date=datetime(2006,5,1),
                                      look_back=10, rnn=None,
                                      type='vanilla', num_internal_projection=10,dropout_probability = 0.2,
                                      init ='he_uniform', loss='mse', optimizer='rmsprop',
                                      nb_epoch=20, batch_size = 10, save_to_csv=None,
                                      train_freq="Monthly", train_period=365, verbosity=True)
                                      
     df_simple_RNN.to_csv('prediction_simple_RNN_v2.csv')
